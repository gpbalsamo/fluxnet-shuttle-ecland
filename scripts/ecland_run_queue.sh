...
# site -- roughly total/N -- so the wall limit has to cover that, and never less
# than the costliest single record; see "WALL LIMIT" in submit_ecland_slurm.sh.
# A limit below the drain means every worker dies mid-queue and nothing is
# recorded: that mistake cost 480 CPU-hours here for zero sites.
#
